import type { Cell, GameState, Mower, PathPoint, ScoreResult, Direction, Achievement, CustomGridConfig } from '../types/game';
...
export function findStartPosition(grid: Cell[][]): { startX: number; startY: number } {
  let startX = 0;
  let startY = 0;
...
const ALL_ACHIEVEMENTS: Omit<Achievement, 'unlocked'>[] = [
  { id: 'speed_demon', name: '疾风割草手', description: '30秒内完成修剪', icon: '⚡' },
  { id: 'perfectionist', name: '完美主义者', description: '完成度达到100%', icon: '✨' },
  { id: 'precision', name: '精准大师', description: '整齐度达到95%以上', icon: '🎯' },
  { id: 'efficient', name: '效率达人', description: '效率评分超过90%', icon: '💪' },
  { id: 'no_overlap', name: '一步到位', description: '路径重叠率低于5%', icon: '🚀' },
  { id: 'slow_steady', name: '稳扎稳打', description: '用时超过2分钟但整齐度90%+', icon: '🐢' },
  { id: 's_rank', name: 'S级园丁', description: '获得S级评价', icon: '👑' },
  { id: 'stubborn', name: '倔强蜗牛', description: '路径重叠率超过40%', icon: '🐌' },
];
...
export function calculateScore(state: GameState): ScoreResult {
  const completion = Math.round((state.mowedCells / state.totalGrassCells) * 100);
  ...
  const total = Math.round(completion * 0.3 + neatness * 0.25 + efficiency * 0.25 + timeScore * 0.2);
  ...
  const achievements = calculateAchievements(time, completion, neatness, efficiency, pathOverlap, total);

  return { time, completion, neatness, efficiency, pathOverlap, total, grade, title, achievements };
}
